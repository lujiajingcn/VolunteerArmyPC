...
| `VA_FF` | **快进倍率**（默认 1）。逻辑层按真实经过时间推进，而车队要到 `CFG.convoyIn = 175` 秒才进地图 —— 不打快进的话，"打到交火"的取证跑图就是 5 分钟真实时间起步。它只放大"喂进来的时间"，步长仍是固定 1/60，所以同一战局秒数下的状态与不快进时一致 |
| `VA_CAPTURE_EV` | **战斗事件当帧自动落盘**：命中标记只亮 0.24 秒，定时截图撞不上，改由事件自己声明"该留证据了" |
