...
| 键 | 型号 | 引擎里的车体尺寸（`spec.len/wid × S`） | 实物尺寸（对照用） |
|---|---|---|---|
| `jeep`  | Willys MB | 2.3 × 1.2 m | 3.33 × 1.57 m |
| `apc`   | M3 Half-track | 3.0 × 1.5 m | 6.18 × 2.22 m |
| `tank`  | M4A3E8 Sherman | 3.7 × 1.9 m | 5.84 × 2.62 m（不含炮管） |
| `truck` | GMC CCKW | 3.3 × 1.5 m | 6.93 × 2.24 m |
...
### ⚠️ 归一化目标是 `spec.len × S`，**不是实物长度**

这是本节唯一一条"照直觉写就会错"的规则，也是这一层与角色 / 武器**最本质的差别**。

`VehicleSpec.len/wid` **同时是 sim 用来判碰撞与遮挡的车体尺寸**
（`va_world.cpp::vehicle_blocked`：`|lx| < len/2 + r/2`，是**全长**不是半长）。
而它比实物短得多 —— 上表第四列就是差距，吉普只有实物的 69%、半履带车只有 49%。

照实物缩放的话，画面上就会出现「**子弹从车厢里穿过去**」这种视线与判定脱节 ——
而 `va_config.cpp` 在 `src/sim/` 里，是**不许动**的。
所以换真模型只改"长什么样"，不改"占多大地方"：目标车长 = `spec.len × S`。
...
| `gen3d.py` / `gen3d_batch.py` | 图生3D 单张 / 批量编排（绕命令行长度上限、按服务端并发配额限流、断点续跑）；`--kind char\|wpn\|veh` 切换档位 |
...
- **载具形象**：车队的四辆车（`jeep`/`apc`/`tank`/`truck`）由**程序化图元**换成三维模型，
  ...
  两条与角色 / 武器**不同**的口径，都写进了代码注释：
  - **归一化目标是 `spec.len × S`，不是实物长度** —— `VehicleSpec.len/wid` 同时是
    `vehicle_blocked()` 判碰撞与遮挡用的车体盒（实测吉普只有实物的 69%、
    半履带车 49%）。照实物缩放会变成"子弹从车厢里穿过去"，而 `va_config.cpp`
    在 `src/sim/` 里不许动 → **换真模型只改"长什么样"，不改"占多大地方"**。
