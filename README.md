...
| `gen3d.py` / `gen3d_batch.py` | 图生3D 单张 / 批量编排（绕命令行长度上限、按服务端并发配额限流、断点续跑）；`--kind char\|wpn\|veh\|vm\|prop` 切换档位 |
...
- **地物四件套的后两件（针叶树 / 灌丛）**：上一轮因"半透明叶簇塌成退化几何"未采用
  （两块岩石已接上）。复做配方见「地物形象」一节：参考图换**不透明**的稠密针叶树 /
  密实灌丛（判据是"枝干之间不透出背景"），生成侧用 `VA_GEN3D_MODEL=hy-3d-3.0`
  换同族档位吃免费额度，跑之前先把 `sweep/gen3d_prop/rejected/` 里那两个删掉
- 载具模型精致化（目前仍是块状体）
- 特效层：曳光弹、爆炸、弹壳抛出、烟尘
