...
sweep/sim/va_sweep.exe campaign   # 五个伏击阵地连着跑（见「五个伏击阵地」）
```
...
| `VA_SCRIPT_A` | **按离线扫描同一套剧本发口令**：`全体，隐蔽` → `爆破手，起爆`（先头车 `x < 1180`，按位置）→ `全体，开火` → … → `全体，撤离`。
  （呼号一律**按职务**发，不写死人名 —— 五个阵地各有一份花名册，写死 `老白` 在第二关之后就没人答应了）。它是 `VA_AUTO` 的**补充而不是替代**：`VA_AUTO` 只顶替玩家的手，而**伏击是由"第一枪"触发的**，车队不进射界就永远没有第一枪 —— 整局会以「车队冲过了西侧出口，伏击失败」收场（实测踩到过，那时画面上一条命中都没有，看着像命中判定坏了） |
| `VA_DOWN_AT` | 到指定战局秒数让玩家被**真实伤害**击倒（走 `damage_unit` → `down_player`，不是把 `downed` 置 true） |
| `VA_CAMP_START` / `VA_CAMP_FORCE` | **战役入口**（引擎侧也认）：从第 N 关起算 / 打输了也往下走一关。⚠️ `VA_CAMP_FORCE` 只是**链路验证开关，不是玩法** —— 带着它跑出来的日志显示"关关过关"，拿它当基线会得出"结果退化了"的错误结论（判据：比对之前先看结尾的"过关 N/M"能不能自圆其说，别只看文件名） |
| `VA_SW_RETREAT` / `VA_SW_KILLPLR` / `VA_SW_CAS` | **离线扫描器的三个验证开关**（引擎侧不认）：提示一弹出就选撤退 / 到点打死玩家 / 交火后立刻减员 N 人。用来确定性复现"撤退"与"换人"两条分支 —— 机械剧本**永远打不到伤亡过半**，没有 `VA_SW_CAS` 那条路离线测不到。见「五个伏击阵地」 |
| `VA_RETREAT_AT` | **到指定战局秒数强制弹出「伤亡过半 → 1 撤 / 2 守」选择条**（引擎侧取证开关）。只置 `W.retreatOffered` 一个标志，**不替玩家做选择** —— 撤还是守、什么时候结算，仍全由 `src/sim/` 的真实代码走完，所以"按 1 之后日志出现「伤亡过半，主动撤出」"这条判据有效。取证脚本 `tools/capture_retreat.sh`（`shot` 拍提示条 / `key 1` `key 2` 实测按键） |
...
| `VA_VM_HAND_ROT` `VA_VM_HAND_LEN` `VA_VM_HAND_PLACE` | 手模的**残余朝向角**（度，"俯仰,偏航,侧倾"，在模型自己的坐标系里）/ 手长（米）/ 落位微调（米）。一次改两只手，扫档用。⚠️ **默认值在 2026-09-24 随换件整体重标为 `90,0,90` / `0.110` / `0,-0.050,0`**：换件时"原始朝向"几乎必然变（旧件腕口朝 +X、新件朝 +Z，而对轴变换一模一样），沿用旧值会得到一个正对相机的大圆筒。新值里 `pitch 90` 是**解**出来的、`roll 90` 是**扫**出来的、`len` 是按**同口径面积**反推的 —— 依据写在 `kVmHandArt` 的注释里 |
...
## 五个伏击阵地（一个阵地打完了，带着人撤向下一个）

单关卡是“一局”，战役是“一条线”。五个阵地全部取自 `铁原阻击战阵地数量.txt`，
是 63 军在铁原以南**逐次抵抗**的五个阻击位置 —— 从最前沿的警戒阵地，一路退到最后一道阻击线。

**每一关只有一条主目标：把敌人钉在这里 240 秒。** 不夺密码箱、不炸坝、不带物件 ——
拖延本身就是任务（`GoalKind::Delay`），熬到 240 秒就转进下一个阵地。
难度不靠加秒数堆：五关都是 240 秒，变的是**敌人一关比一关多**（步兵 10~14 → 16~22）
和**地形一关比一关难受**（山口 → 丘陵 → 孤山 → 峡谷）。

关卡数据集中在 `src/sim/va_campaign.cpp` 的 `L1..L5`（`LevelDef`）：
...
| 关 | 阵地 | 日期 | 主目标 | 敌方步兵 | 敌进入 | 本关时限 | 地形 |
|---|---|---|---|---|---|---|---|
| 1 | 玉女峰 · 前出警戒阵地 | 1951.5.30 | 拖住敌人 240 秒 | 10~14 | 165 s | 275 s | 山口 |
| 2 | 233.2 高地 · 涟川以南第二道阻击线 | 1951.6.1 | 拖住敌人 240 秒 | 12~18 | 160 s | 275 s | 丘陵 |
| 3 | 种子山 · 五峰寺以北 | 1951.6.2 — 6.3 | 拖住敌人 240 秒 | 12~18 | 160 s | 275 s | 丘陵 |
| 4 | 鹫峰 · 第二道防线左翼 | 1951.6.3 | 拖住敌人 240 秒 | 14~20 | 150 s | 275 s | 孤山 |
| 5 | 沙宗洞 · 最后一道阻击线 | 1951.6.5 — 6.6 | 拖住敌人 240 秒 | 16~22 | 150 s | 275 s | 峡谷 |

**“本关时限 275 s”比“目标 240 s”宽 35 秒，这个余量是必需的**：`update_flow()` 的“超时判死”
与 `check_end()` 的“拖延达成”是两条独立的判据，若两者同刻，谁先跑到取决于帧率与快进倍率 ——
会出现“240 秒明明熬到了，却因为同帧先判超时而算失败”。留出 35 秒后两者不再抢。

### 每一关都是史实里的一个阵地

- **第一关 · 玉女峰**：187 师进入阵地当夜即派警戒分队前出二十公里骚扰，迫使美军于 6 月 1 日
  提前展开进攻，为军主力抢修工事抢出一天。对应史实里“前出警戒”这一层，所以敌人只是搜索分队。
- **第二关 · 233.2 高地**：561 团 3 营在涟川山口一线坚守四天三夜，抗击数倍于己的敌人十余次
  进攻，毙伤美军 1300 余人，战后被授予「守如泰山」锦旗。233.2 高地一丢，涟铁公路就通了。
- **第三关 · 种子山**：6 月 2 日激战竟日，种子山、五峰寺及以南阵地相继失守；当夜 566 团以
  一连、三连各一个排组成敢死队，3 日凌晨夜袭种子山，全歼守敌夺回阵地。
- **第四关 · 鹫峰**：6 月 3 日拂晓美 25 师 3 个团加入战斗，以坦克为先导突入纵深；189 师
  战斗减员严重，坚守到天黑才奉命把阵地移交给军预备队 188 师 —— 对应关卡文案里那句
  “你们的价值是**时间**，不是战果”。
- **第五关 · 沙宗洞**：6 月 6 日敌以一个营分两路迂回 207 高地，563 团 1 连 2 排三面受敌、
  背后是悬崖，战至午夜只剩 8 人，弹药耗尽后跳下悬崖，5 人牺牲、3 人生还，
  后称「宁死不屈的八勇士」。所以这一关 `earlyFlank = true`（侧翼迂回来得早）。

### 每个阵地一份花名册：50 个名字，模型复用

五个阵地各有一份 **10 人编制**（`POSITION_ROSTERS`，`src/sim/va_config.cpp`），
共 **50 个事先定好的名字**。职务排列固定：队长 / 机枪手 / 狙击手 / 步枪手 ×2 /
反坦克手 ×2 / 爆破手 / 医疗兵 / 弹药支援 —— **人物模型按职务复用那 11 套 `char_*`**，
`RosterDef::art` 存的就是模型键，所以 50 个名字只对应 11 个外观。

`ROSTER` 因此从**编译期常量表**改成**运行时容器**：进哪一关就装哪一关的人。
它一变，三处跟着变（这是这次改动里最容易被漏掉的一类）：

- **`rebuild_roster_index()`**：分组（`GROUPS`）与职务呼号（`ROLE_CALL`）按**当前花名册**重算。
  否则语音喊“机枪手”时会去喊一个不在场的人 —— 语法解析成功、没有任何报错，只是没人答应。
- **`make_aliases(name)`**：别名改为按姓名现场生成（去掉姓 + 全名），所以 `aliases` 的类型
  从 `const char*` 改成 `std::string`（它没有静态寿命了）。`va_parser.cpp` / `va_commands.cpp`
  里所有遍历都要跟着改，编译器会逐个点名，正好是清单。
- **HUD 的胸像缓存与名册格子**：改成按**模型键**缓存（`art_port_`）而不是按 10 个写死的 id，
  否则第五阵地的人会顶着第一阵地那个人的脸。
- **`va_commands.cpp` / `va_flow.cpp` 里写死的人名**（`laobai` / `shitou` / 铁头 / 小满）全部改成
  **按职务找**：炸桥炸坝找“爆破手 → 反坦克手 → 带炸药者”，密码箱战果归“爆破手”。
  写死 id 在换花名册之后会静默失效（找不到人 → 没人去炸）。

### 跨关继承：活下来的带走，缺的就地补到 10 人

带人判据是**“还活着就带走”**（不是“走到撤离点”）：收拢那一刻站着的和倒着的都进下一关，
阵亡的不复活。补员分两轮：

1. **只在有继承队伍时才跑“缺职务优先”** —— 第一关满编时花名册本身完整，走这一轮会把
   顺序打乱（实测会跳过第二个步枪手、先塞反坦克手）。
2. **再按名册原序填满到 10 人。**

⚠️ 第一条的 `if (carried)` 不是可有可无的：**建队顺序决定 RNG 抽取顺序**，
满编开局走“缺职务优先”会把十种子扫描的胜率与伤亡整体平移一格，新数据再也比不了旧基线。
（离线基线的另一半也在 `RECOMMEND`：那份表的**坐标与顺序是逐项照抄旧花名册同槽同职务**的，
动一格同样会平移基线。）

两个倒地相关的口径（都是踩出来的，不是设计出来的）：

- **队友倒地带进新关时 `downTimer = 120`**（原本沿用新兵的 45 秒）。继承的伤员开局还没接敌
  就集体断气 —— 实测第四关 45 秒就被判“伤亡过半”，看着像平衡崩了，实际是倒计时没跟着继承走。
- **玩家不带倒地进新关**，按“带伤起立 35% 血”处理。玩家倒地时操作权是被收走的，
  带着这个状态进新关等于开局不能动。

### 伤亡过半：1 撤 / 2 守

交火开始之后，只要**伤亡（阵亡 + 倒地）达到开局人数的一半**，屏幕中央弹一条**常驻选择条**：

```
伤亡过半 ——  1  撤向下一阵地      2  继续死守
```

- **按 1**：立刻 `capture_carry()` + `commit_level_result(false)` + 转进下一阵地。
  战报写明“只拖住 N 秒” —— 提前撤出就是没熬满，这点不含糊。
- **按 2 / 不按**：继续死守，提示条收起，这一关打到 240 秒或打到全灭为止。

提示条是**常驻**的，不是会淡出的 toast：这是一个等玩家决策的状态，不是一条通知。
（警报条和 toast 都会自己消失，用它做选择条会出现“提示还在、按键已经没用了”的错位。）

**实机取证**（离线侧只能证"逻辑判据对"，证不了"HUD 画得对、按得动"）：
`tools/capture_retreat.sh`，两种用法 —

```bash
tools/capture_retreat.sh shot        # VA_RETREAT_AT=150 强制弹出，拍 152/170/200 三张
tools/capture_retreat.sh key 1       # 弹出后真实投一次「1」，打印链路证据
tools/capture_retreat.sh key 2       # 同上，投「2」
```

实测（种子 20260925）：`shot` 的三张里选择条**都在**（弹出后 50 秒仍未消失，证实"常驻"）；
`key 1` 的日志出现
`[结算] 转进：伤亡过半，主动撤出 玉女峰 · 前出警戒阵地（只拖住 173 秒）。转进 233.2 高地…`；
`key 2` 的日志出现的是
`[结算] 转进：玉女峰 · 前出警戒阵地 拖住敌人 240 秒。转进 …` —— **没有**"主动撤出"字样，两条分支分开。

⚠️ 脚本里轮询的判据必须是 `retreat_inject()` 在**触发时刻**打的那行（`t=… 强制弹出撤退选择条`），
不能是 `_ready` 里的「取证注入：…」—— 后者游戏一起步就打了，拿它当判据会把按键投在
弹出之前（实测第 3 次轮询就"已弹出"，实际战局才 t≈20，按键落空）。

### 玩家阵亡：直接去下一个阵地，换那个人

玩家一死，`kill_unit()` 里走 `on_player_lost()`：**就地收拢 + 结算本关 + 转进下一阵地**，
下一阵地由**该阵地的队长**接任（名册 `men[0]`），姓名跟着换。

所以“换人”是**阵地换人不换队**：带过去的那 9 个人还在，只是你换成了新阵地的队长。
实测五关依次是 李长顺 → 高德明 → 郭守成 → 杨建国 → 阎成福，其余 9 人原样跟着走。

如果这一关就是最后一个阵地，则直接判失败（"你在最后一个阵地上阵亡了"）。
**玩家没死就继续控制原来那个人** —— 这条是默认路径，不需要额外代码，
但它是需求里明确的一句，值得单独写下来：不能因为“换人”这条新路，把继承的那条老路挤掉。

### 密码箱下线

战役模式下 `W.boxWhere = "none"`，`build_convoy()` 的守卫改成
`if (W.boxWhere == "truck" || W.boxWhere == "apc")`，`boxAlways = false`、`evacNeed = 0`。
**单关模式仍然有密码箱** —— 那是 `levelIdx < 0` 那条路，与战役无关，不要一起删。

### 打输了会怎样

- **转进**（这一类不算输）：240 秒熬到了、还有下一关。结算面板写“按 R 转进下一阵地”，
  **不显示 S/A/B/C 徽章** —— 战役还没打完，不该给一个终局的评分。
- **失败**：全灭、或提前撤出到最后一个阵地、或在最后一个阵地阵亡。写“按 R 重打本关”，Esc 回主菜单。
- 第五关之后没有下一关，落点是史实的 6 月 12 日 19:30“63 军奉兵团命令转向伊川地区休整”。
...
```
sweep/sim/va_sweep.exe campaign 7
```

```
关  阵地                 结果  活  亡  倒  撤  带 用时  目标
     名册(10)：李长顺*[队长] 王铁柱[机枪手] 赵永年[狙击手] 陈二狗[步枪手] 孙有田[步枪手] 周大勇[反坦克手] 吴小山[反坦克手] 郑满仓[爆破手] 韩秀英[医疗兵] 马德胜[弹药/支援] （* = 玩家）
1    第一关 · 玉女峰 转进   10    0    0    9   10   240s  [√]拖住敌人 240 秒
     × 结束原因：玉女峰 · 前出警戒阵地 拖住敌人 240 秒。转进 233.2 高地 · 涟川以南第二道阻击线 —— 63军187师561团3营。
2    第二关 · 233.2 高地 转进   10    0    0    7   10   240s  [√]拖住敌人 240 秒
3    第三关 · 种子山 转进    8    0    2    8   10   240s  [√]拖住敌人 240 秒
4    第四关 · 鹫峰    转进    5    4    1    6    6   240s  [√]拖住敌人 240 秒
     名册(10)：李长顺*[队长] 赵永年[狙击手] 孙有田[步枪手] 郑满仓[爆破手] 韩秀英[医疗兵] 马德胜[弹药/支援] 尹黑牛[机枪手] 翟铁蛋[反坦克手] 冷长贵[狙击手] 江大水[步枪手] （* = 玩家）
5    第五关 · 沙宗洞 胜利    8    1    1    7    -   240s  [√]拖住敌人 240 秒

→ 过关 4/5   累计阵亡 3   累计撤离 37 人次   总用时 1200 秒
```

第四关那份名册就是“继承 + 补员”的样子：前 6 个是活下来跟过来的（队长、狙击、步枪、爆破、
医疗、弹药），后 4 个是沙宗洞就地补的（机枪、反坦克、狙击、步枪），**合计恒为 10**。

三个验证开关（只有离线扫描器认，不影响玩法）：

```bash
sweep/sim/va_sweep.exe campaign                  # 从第一关连续跑
sweep/sim/va_sweep.exe campaign 101              # 换种子
VA_SW_RETREAT=1 sweep/sim/va_sweep.exe campaign  # 伤亡过半的提示一弹出就选「撤退」
VA_SW_KILLPLR=180 sweep/sim/va_sweep.exe campaign # 180 秒打死玩家，验「换人」
VA_SW_CAS=5     sweep/sim/va_sweep.exe campaign  # 交火后立刻减员 5 人，把队伍打到伤亡过半
```

`VA_SW_CAS` 是必需的：机械剧本**永远打不到“伤亡过半”**（最硬的一局也只伤亡 3/10），
没有它，撤退那条分支在离线侧根本无法确定性复现。

**引擎层那一条链只能真跑一遍才验得到**（离线工具没有 HUD、也没有按键）：
`VA_CAPTURE` 先把撤退提示条拍下来，再按 1 / 2 各走一次 ——
日志里应分别出现“伤亡过半，主动撤出 …”与继续打到 240 秒。

### 编制 11 → 10：基线为什么会整体平移

旧花名册是 **10 名队友 + 玩家 = 11 人**；新编制是**每阵地 10 人（含玩家）**。
少一个人的直接后果是十种子扫描整体右移一格：

| 档位 | 旧（11 人） | 新（10 人） |
|---|---|---|
| BAL 默认 | 6/10 · 伤亡 56 | 6/10 · 伤亡 53 |
| base | 2/10 · 伤亡 76 | 1/10 · 伤亡 75 |
| A-mg | 6/10 · 伤亡 56 | 6/10 · 伤亡 53 |
| B-hp | 4/10 · 伤亡 58 | 5/10 · 伤亡 53 |
| C-evac | 3/10 · 伤亡 74 | 2/10 · 伤亡 85 |
| A+B | 7/10 · 伤亡 41 | 5/10 · 伤亡 51 |
| A+C | 6/10 · 伤亡 59 | 7/10 · 伤亡 56 |
| P1-hp18 | 7/10 · 伤亡 36 | 8/10 · 伤亡 28 |
| P2-mg45 | 7/10 · 伤亡 39 | 6/10 · 伤亡 41 |
| P3-mg45hp | 8/10 · 伤亡 25 | 7/10 · 伤亡 25 |
| P4-mg40hp | 8/10 · 伤亡 35 | 8/10 · 伤亡 35 |

**这是需求 2 的必然结果，不是回归。** 实际生效的 `BAL 默认` 档仍是 6/10，与改动前一致。
（战役侧不受这条影响 —— 战役的胜利判据是“熬满 240 秒”，不是撤出几人。）

**「撤」与「带」是两列，不是一列。** `撤` 是结算那一刻真从撤离点走出去的人数（统计口径），
`带` 是实际被继承进下一关的人数（继承口径）。本作带人条件是“还活着”，所以不变式是
**`带 == 收拢那一刻的（站 + 倒）`**；停表时刻与收拢时刻同帧结算时两者会差一点，那只提示不报错。
这条不变式抓出过一个真 bug：`capture_carry()` 原来在收拢末刻**拿“当前距离”再量一次**，
...
> ⚠️ **2026-09-24 修正**：上面那个 0.554% 的噪声底是在**画面里没有摆动的地物**时测的。
> 同一台机器、同一个出生点（旁边那棵树冠**随风摆**）再测，**同参数连拍两张**的满屏差
> >8 有 **4.022%**、手部 ROI 内 **29.495%**。所以：
> ① 那条 0.554% **不能当通用噪声底**用，用之前先在本场景实测一次；
> ② 本场景里**逐像素 A/B 已经失效**，要判"手有没有变"得改用**固定 ROI 的颜色中位**
> （连拍 4 次散布仅 0.2 L）。详见下面「2026-09-24 换件」那节的三条量测铁律。

#### 2026-09-24 换件：从「图生3D 空手握拳」换成「文生3D 土黄棉手套」

**为什么要换**：09-21 那个模型的几何是**一整团钝圆体** —— 识别色图下手指只在轮廓上有
浅浅起伏，"指头的形状"完全靠贴图糊出来，放大 3 倍还能看到手背上一块贴图白斑。根因与
「树冠叶簇」**同构**：图生3D 是**单视图重建**，而"握拳"这个姿势里**指缝之间的空腔在任何
单张照片里都看不见**（能看到的永远是"手指叠在一起的那一面"），重建只能把那些区域蒙成一块。
→ 改走**文生3D**（`tools/gen3d_batch.py --kind vm` 的 `prompts` 表，prompt 全文见
`ref/vm/SOURCES.md`）。

两个候选**一次全成**，几何判据（`tools/vm_hand_geo.py`，口径从 `kVmHandArt` 表反抽）：

| 候选 | prompt 主语 | 长轴/次长轴 | 判读 |
|---|---|---|---|
| v2 | 五指分开的手掌 | **1.652** > 1.45 | 拦下：五指张开 + 一块方柱底座 |
| **v3** | 攥成拳头的厚棉手套 | 1.27 | **采用**：握拳，带"腕口台座"（占长轴约 25%） |

![双手换件前后 · 左 09-21 图生3D（腕口开口外露、手指只有贴图画出来的一道褶）/ 右 文生3D 棉手套](docs/vm_hand_glove_ab.png)

**换件之后 `rot` / `len` / `place` 必须整体重标**，因为**新旧件的原始朝向不同**：
两件都在 `rot=0` 下各拍一张并排看 —— 旧件腕口朝 **+X**（横躺），新件腕口朝 **+Z**（正对相机），
而两者的对轴变换**完全相同**（日志"对轴后"都是 Y→Z、Z→Y）。拿旧件的 `rot` 直接套，
画面里出现一个大圆筒（腕口切面正对相机）。
新参数：`rot = 90,0,90`（`pitch 90` 是**解**出来的：腕口 +Z → −Y；`roll 90` 是**扫**出来的：
让四根手指**横向**排列，`roll=0` 读出来是"举手"而不是"攥住"）、`len = 0.110`
（按**同口径面积**反推：`0.105 × √(87547/78695) = 0.1108`，不是拍的）、
`place = (0,−0.050,0)`（新件长轴里含腕口台座，包围盒中心比旧件更偏腕侧；不补这 5 cm
手是**浮在枪右上方**的）。

> ⚠️ 日志里那行"对轴后 (0.668, 1.053, 0.835)"**只说明包围盒被置换**，在 90° 整倍数旋转下
> 它**不含朝向信息**（等价于 `(0.835,1.053,0.668)` 的一个重排）。所以"代码默认版"与
> "env 覆盖版"的朝向是否一致，**不能看这一行** —— 见下面那条 A/B。

**材质重标：`tint` 从 0.50 抬到 0.71，而且"越亮越像棉布"是错的。**
旧值 `0.50,0.42,0.36` 的前提是"**贴图是皮肤色**，要把它乘成皮革棕"；新模型的贴图
**本身就是深赭黄的粗棉布**（把 `baseColorTexture` 导出来看过：米黄棉布底 + 编织罗纹 + 缝线，
**没有皮肤色**），再乘 0.50 是**二次着色**，读成"深棕皮手套"。但抬到 0.80 又过了 ——
亮面 R 通道压到 **233** 接近饱和，**布纹被冲平**，读成"光滑乙烯基"。
判据因此不是"够不够亮"，而是**亮面还留不留得住布纹**。四档扫描（同一机位、同一 ROI
`x1500-1700,y1000-1160` 手套受光面、逐通道中位）：

| tint | 手套受光面 | 观感 |
|---|---|---|
| `0.56,0.49,0.38` | `RGB(129, 97, 83)` L103 | 灰褐，偏脏 |
| `0.63,0.56,0.42` | `RGB(157,115, 88)` L122 | 浅卡其，饱和度不够、发白 |
| **`0.71,0.61,0.43`** | **`RGB(193,133, 94)` L142** | **最贴"土黄"，腕口罗纹在 1.55 倍缩放下仍可见** |
| `0.80,0.68,0.45` | `RGB(233,162,102)` L173 | 金黄偏琥珀，亮面纹理被冲掉 |

（参考：场上草地约 `RGB(172,170,129)` L167 —— 采用档的手比背景草略暗但**色调分离明确**，
不会糊进环境。教训：**"色调乘数"只在"贴图是另一种材质"时成立**；贴图已经是对的颜色时，
同一个数就从"补偿"变成了"污染"，换件之后必须重量。）

**开镜：从"缩 0.65 让位"改成"缩到看不见"。** 0.65 那一档是拿**程序化手套**标定的 ——
它没有内部结构，缩到 35% 仍读作"握着的拳头"。换成真手模之后同一档读出来的是
**一块悬在机匣右侧的橙色残片**（腕口朝下像个小支架，不握任何东西）。而"干脆别缩"也不行：
开镜时相机在枪轴上、离握把只有约 **22 cm**，实尺手模 0.11 m 宽在这个距离上占 **0.55 屏高**。
→ 所以保留"缩小"这条通路、把终点设成 0（缩放连续，不突跳），并在低于 `0.03` 时把
`hands` **整组藏掉**（避开 0 缩放的退化变换，也免掉 3% 尺寸上的一堆子像素抖动）。
`VA_VM_ADS_HSHRINK` 的上界因此从 0.85 放到 1.0，默认 **1.0**。
注意这条**不是**本轮新引入的退化：09-21 那版模型在开镜里**也是同一块悬浮残片**
（`sweep/vm6/now_mosin_ads`），只是颜色暗、不容易被注意到。

![开镜 · 左 改前（悬浮手套残片 + 前臂圆柱）/ 右 改后（双手收掉，只剩枪、照门、目标）](docs/vm_ads_hands.png)

**本轮新增的量测铁律（三条，都会让"看起来变了"变成假警报）**

1. **本场景里"逐像素 A/B"已经失效**：出生点旁边那棵树的树冠**随风摆动**，而摆相位跟墙钟走。
   实测**同参数连拍两张**：满屏差 >8 的有 **4.022%**、手部 ROI 内 **29.495%**
   —— 比"跨时间不同参数"那一组还大。所以**不要**再拿满屏 A/B 去判"手有没有变"。
2. **改用"固定 ROI 上的颜色中位"**：它对 1 像素抖动稳健。同一 ROI 连拍 4 次的 L 中位是
   **142.6 / 142.4 / 142.5 / 142.5（散布 0.2）**，而 tint 每档间距是 19~31 —— 判据信噪比足够。
3. **`vm_hand_probe.py` 的"枪身"那一行在本场景不可信**：`|常规 − 藏VM|` 这个掩码会把
   **摆动的树**一起圈进来（藏 VM 也改了 VM 投在世界上的影子），两次运行同一份配置得到
   `RGB(103,86,100)` 与 `RGB(154,114,96)` 两个差很远的"枪身色"。**只有被识别色门控的
   "手套 / 袖子"两行可信**（它们同时要求该像素在识别色图上是手套/袖子的颜色）。

**最终数字**（莫辛腰射，同掩码口径，`sweep/glove/`）：

| | 像素 | 占比 | 中位 L | 说明 |
|---|---|---|---|---|
| 手套 | 82278 | 2.35% | **119** `RGB(162,109,86)` | 换成真棉手套后与旧件（86107 / 2.46%）**几乎同占屏** |
| 袖子 | 31212 | 0.89% | 42 | 与上一轮逐位相同（前臂那条链没被带动） |
...
场上**数量最多**的一类东西。各关的 `ThemeMix` 给每关约 **25~40 块岩石、10~20 棵树、
...
### 树干是品红色：一条**颜色**判据，和一次「零额度胜过重新生成」的裁决（2026-09-24）

出生点就在树底下，第一人称一抬头就是**一整面品红的树干**，占掉大半个视野，
与满屏的灰绿土黄刺眼地打架。它从 09-23 那版起就在，**一直没被发现**——
因为上一轮的判据只判**形状**（剖面锯齿度 0.2120 → 0.0268 全过）。
**"塌了 / 分层 / 颜色错"是三种不同的坏法**，形状判据看得见的只有前两种。

根因有两层，第二层是这一轮才挖出来的：

1. 生成器给的 `baseColorTexture` **底色本身就是暗红**（红底 + 灰绿枝叶团块，像迷彩）
   —— 不是引擎问题、不是 tint 问题。
2. **prompt 里明写着"红褐色树干"**，末尾还写着"东北**红**松"。
   生成器忠实执行了它 —— 这不是"生成器抽风"，是**描述里真的带着色相指令**。

> 为什么不能在引擎侧改：红与绿在**同一张贴图**里。tint / albedo 是整张贴图的乘数，
> 改它会把树冠一起染红。要在像素级分开，只能在这一层做。

**补的颜色判据**（`tools/glb_tex_color.py`）：贴图逐像素转 HSV，只统计有色像素，
按色相分带报占比。判据是**红系占比**（色相 ≥330° 或 ≤25°）——
正常树皮该落在 20°~45°（橙棕/黄褐）那一段。与几何判据同一个道理：**看相对高低，没有绝对阈值**。

然后**两条路并行**，各出一件（`v4` 花掉 1 次额度 = 40 积分，离线那条零额度）：

| 候选 | 做法 | 贴图红系 | 锯齿度 | 几何 | 树干观感 | 结论 |
|---|---|---|---|---|---|---|
| `v3`（现役） | —— | 8.7% | 0.0268 | 基准 | 一整面品红 | 问题件 |
| `v4`（重新生成） | v3 的**形状措辞逐字保留**，只把"红褐色树干"换成"深灰褐色混着青灰与土黄"、去掉"东北**红**松"的红字 | **0.0%** | 0.0275 | 合格（没退化） | **灰白**，读作水泥柱 | ❌ |
| `g_B`（离线修贴图） | `tools/glb_tex_recolor.py` 只把**红系像素**的色相/饱和/明度重映射，网格一个顶点不动 | **0.1%** | **0.0268** | **逐位不变** | **深棕树皮** | ✅ 采用 |

**被数据推翻的第二个直觉**：我以为"重新生成能修颜色"，结果生成器给的树干是**灰白偏冷**的，
比品红好、但远不如离线改的棕褐自然。而离线那条**连上一轮好不容易拿到的几何成果
（1/7.9 的锯齿度）都完整保住了** —— 重新生成等于**重掷骰子**。

**量出来的三组数**（同种子、同机位、同 `VA_UNIT_SHOW=prop:prop_pine`）：

| 量 | `v3` 品红 | `g_B` 定稿 |
|---|---|---|
| 贴图红系占比 | 8.7% | **0.1%** |
| 贴图橙棕占比 | 12.6% | 21.1% |
| 贴图绿占比 | 63.7% | **63.7%（一个像素未动）** |
| 树干 ROI 红系（`R−G≥25`） | 28.2% | **0.0%** |
| 树干 ROI 强红（`R−G≥60`） | 6.3% | **0.0%** |
| 树冠 ROI 中位 | `RGB(119,162,154)` | `RGB(119,162,154)`（**逐位相同**） |

> ⚠️ **"保留明度"会让树干读作水泥柱。** 第一版（保 V、饱和 ×0.55）量出来红系已经归零，
> 但画面里树干是**浅灰**的 —— 同一 V 下红→黄褐的**感知亮度是上升的**，再叠上降饱和
> （接近灰），受光面直接发白。修法是明度也按比例压（`--lift 0.78`）。
> 这是"越调越像"这条路上的又一个假里程碑：**判据归零 ≠ 观感到位**。

**顺带做了一次全素材颜色体检**（21 个 GLB）：只有 `prop_pine` 是问题件。
`prop_rock_a` 红系 26.4%、`wpn_ppsh` 21.6%、`wpn_dp27` 12.4% —— 这些是裸岩与木枪托的
自然色（它们同行的**橙棕占比**分别是 62.6% / 78.3% / …），**光看"红系高"会误伤**，
要和橙棕一起看。
...
| `glb_tex_color.py` | GLB 贴图的**颜色验收**：`baseColorTexture` 逐像素转 HSV，只统计有色像素（`--sat` 控门槛），按色相分带报占比 —— 判据是**红系占比**（色相 ≥330° 或 ≤25°，正常树皮该落在 20°~45°）。它拦的是几何判据全过、**颜色却错**那一类（实例：`prop_pine` 的品红树干，上一轮只判了锯齿度所以漏掉了）。`--dump` 顺便导出贴图与红掩码（掩码回答"红在哪"，只报占比会分不清"整块红底"与"零星被染到的缝"）。⚠️ **红系高不一定是错**：要和同行的橙棕占比一起看（裸岩 26.4% 红 / 62.6% 橙棕、木枪托 21.6% 红 / 78.3% 橙棕，都是自然色） |
| `glb_tex_recolor.py` | **零额度**地修贴图颜色：把命中"红圈"的像素的色相拉向 `--target-h`、饱和度乘 `--sat-mul`、明度乘 `--lift`，其余像素**一个都不动**。网格 / 法线 / UV 全不碰，所以**几何判据逐位不变** —— 这才是它相对"重新生成"的价值：不重掷骰子。要重排 BIN 并回写每段 `byteOffset`（新 JPEG 长度和旧的不一样，原地覆盖会把后面的 normal/mr 贴图顶掉）。⚠️ **明度不能"原样保留"** —— 保 V 会让树皮读作水泥柱（同一 V 下红→黄褐的感知亮度是上升的）。用法：`glb_tex_recolor.py <in> <out> [--target-h 30 --sat-mul 0.75 --lift 0.78] [--dump <目录>] [--dry-run]`。**改颜色一律从"带问题的原件"改**（存 `sweep/pinecolor/rejected/`），不要在上一档的产物上叠加 |
...
**先说清楚素材限制。** 场上 11 个角色模型全部是**单块融合网格**：

```
char_rifleman.glb   nodes=1 meshes=1 prims=[1] mats=1  skins=0 joints=0 animations=0
                    attrs=[NORMAL, POSITION, TEXCOORD_0]
```

没有骨架、没有蒙皮、没有动画轨、没有骨骼权重 —— **不能用骨骼驱动**任何关节。
这句话曾经被写成"腿前后摆做不到"，那是把一条实现路径（骨骼）的限制当成了能力边界。
实际上还有第二条路：**着色器顶点位移**，本层走的就是它。

> **⚠️ 更正**：本节早先与 `unit_anim.h` 里"当前素材下做不了腿摆"的结论**已被推翻**。
> 推翻它的不是新素材，而是一个实测出来的几何事实（工具 `_inspect_glb_legs.py`，
> **先套 root 节点上的 +90°X 旋转再统计**）：腿区间（身高下 35~45%）里
> **跨中线的三角形 = 0 个（0.0%）**，中轴 ±2% 身高范围内**几乎没有顶点**。
> 即左右腿本来就是**几何上分离的两簇**，中间有一条空谷 —— 按 `sign(x)` 判左右
> 既可靠、又不会把网格撕开，**不需要骨架就能分腿**。

于是本层做成**两层叠加**，各管一件事：

| 层 | 管什么 | 施加在 | 实现 |
|---|---|---|---|
| `UnitAnim` | **整体**跑动节奏：垂直起伏 / 前倾 / 左右摇 / 肩摆 / 点头 | 单位节点的变换（`unit_transform * local_pose`） | `src/node/unit_anim.cpp` |
| `UnitLeg` | **腿部**前后交替：髋以下按高度权重绕横轴旋转，左右腿相位差 π | 网格顶点（着色器 `vertex()`） | `shaders/unit_leg.gdshader` + `src/node/unit_leg.cpp` |

两层共用同一份相位（`UnitAnim::PoseVals::leg_phase / leg_deg`，由 `pose_at()` 算出），
所以不会出现"腿在摆而躯干不动"的脱节，静止 / 起步 / 停下的淡入淡出也自动一致。
**逻辑层 `src/sim/` 一行未改。**

判据不是"好看"，而是"能不能读出人在跑"：站立的人不会周期性上下颠、不会前倾、不会左右摇 ——
三条同时出现，几十像素高的远处士兵也会被读成"在跑"；腿摆让近景更经得起看。

### 整体跑动节奏（`UnitAnim`）

| 项 | 做法 | 出处 |
|---|---|---|
| 速度来源 | **相邻两帧逻辑坐标差 ÷ dt**，不是 `Unit.moving` / `vx,vy` | 位移还有编队 lerp（`va_ai_ally.cpp:76`）与出生摆放（`va_world.cpp:290`）两条路不走 `move_step`，用 `moving` 会判出"在跑但不动" |
| 时间基 | `va::W.t`（战局秒），**不是墙钟** | `VA_FF` 快进时动作与逻辑同步加速；检阅台 / 菜单期 `t` 冻结，动作自动停 |
| 起步相位 | 复用 `Unit.wobble`（原本**只写不读**的死字段） | 它在 `va_ai_ally.cpp:36` 每帧 `+= dt*6`、`va_world.cpp:305` 出生时随机播种 —— 天然是一人一个相位，队伍不会同手同脚 |
| 幅度包络 | 0.35 m/s 以下不动，1.15 m/s 到满，指数趋近 | 站桩 / 微调站位时不该抖 |
| 施加方式 | `unit_transform(...) * anim_.local_pose(i)` —— **右乘** | `unit_transform` 仍独占"人在哪 / 朝哪 / 倒没倒"，跑动层只加局部摆动；倒地与朝向的原有判据不受影响 |

**坐标口径（本层最容易错的一处）**：`local_pose` 工作在单位模型自己的局部系里 ——
原点在**脚底**、前方 **+X**、上方 +Y、侧向 ±Z。所以三个轴各管一件事，别用错：

| 轴 | 语义 | 用在哪 |
|---|---|---|
| 绕 **Z** | 俯仰（前倾 / 点头） | 前倾 4°~10° 随速度，另叠 ±2° 点头 |
| 绕 **X** | 侧倾（左右摇） | ±3.5°，步态频率 |
| 绕 **Y** | 偏航（肩部摆） | ±3.5°，与左右摇差 90° —— 跑起来是"拧"的 |

**现场可调（不重编译）**：`VA_RUN=0` 整体关；
`VA_RUN_AMP` / `VA_RUN_HZ` / `VA_RUN_BOB` / `VA_RUN_LEAN` / `VA_RUN_ROLL` / `VA_RUN_YAW` / `VA_RUN_NOD`
分别覆盖总幅度、满速步频（默认 1.35 Hz）、起伏（0.055 m）、前倾、左右摇、肩摆、点头。
`VA_DBG_RUN=1` 每 0.5 秒墙钟打一行「战局秒 / 跑动人数 / 最快地速」，收尾再打一次峰值。

```bash
# ① 跑动排（首选）：同一模型按一个步态周期均分相位摆一排，
#    一张**静态**截图看完整个循环 —— 相位是给定值，不依赖帧对齐、不需要 FF。
VA_UNIT_SHOW=run:char_rifleman VA_CAPTURE=1 VA_CAPTURE_DIR=res://sweep/row_on  ...
VA_UNIT_SHOW=run:char_rifleman VA_CAPTURE=1 VA_CAPTURE_DIR=res://sweep/row_off VA_RUN=0 ...
python tools/diff_png.py sweep/row_on/cap_1s.png sweep/row_off/cap_1s.png out.png --thresh 8
#    近景 6.6 m / 6 格，默认幅度下开/关差异 **16.19%** 像素（横跨整排 6 人）；
#    把 VA_RUN_LEAN 抬到 45° 可以一眼确认姿态方向对不对（差别大到不会误判）。
#    VA_RUN_DEMO_SPEED / VA_RUN_DEMO_N 控制演示速度与格数（默认 满速 / 6）。

# ② 实跑消融（看战场里真的有没有单位被驱动）—— 三个前置条件缺一不可：
#    必须 VA_FF=1：acc_ += p_delta*ff 里的 p_delta 是真实帧时，ff>1 时两次运行帧数
#      不对齐 → 截到的战局时刻差零点几秒 → 整个车队都位移了，差值里全是"世界动了"。
#    必须 VA_HIDE_HUD=1：罗盘/雷达/闪烁提示是墙钟动画。
#    必须 VA_VM_HIDE=1：枪模摇摆也是墙钟驱动。
#    （实测：不摘 HUD 时基线噪声 0.53% ≈ 信号 0.06%，完全判不出来；
#      摘掉 HUD+枪模后基线 **0 像素**——同种子两次运行逐字节一致。）
VA_SEED=20260925 VA_FF=1 VA_HIDE_HUD=1 VA_VM_HIDE=1 \
  VA_CAPTURE="5,6,7,8,9,10" VA_CAPTURE_DIR=res://sweep/ab2_on  ...
#    ⚠️ 这条路的信号可能很小：**在动的单位不一定在镜头里**（实测某次仅 19 像素）。
#       要证明"确实在跑"，先看 VA_DBG_RUN 的姿态分量数字，再看像素。
```

> **一条踩过的坑：纯函数也要认总开关。** `pose_at()` 是给检阅台用的无状态函数，
> 最初没检查 `enabled_` —— 于是 `VA_RUN=0` 时**两边照样都摆了姿态**，
> 开/关对照只剩 15 像素噪声，看起来像"姿态根本没生效"。
> 修法是在纯函数开头也判一次 `enabled_`。凡是"能被外部直接调用的纯函数"，
> 都要自己认开关，不能指望调用方替它判。

**验证（`VA_SEED=20260925`）**

- **数值**（`VA_FF=1 VA_DBG_RUN=1`）：日志直接打出最快单位的姿态分量，与设计公式**逐项吻合** ——
  地速 1.38 m/s（= 46 单位/秒 × 0.05 × 0.6，正是"接近目的地减速"那一档）
  → 前倾 = 4 + 6×0.6 = **7.6°**，实测 7.6°；起伏在 −0.049 ~ +0.055 m、摇 ±3.5°、摆 ±3.4° 之间往返。
- **驱动面**：18 个单位跑满一局（FF=8），64 次采样中 **57 次有单位实际移动**，
  同时最多 7 个在跑；峰值 4.95 m/s（交火瞬间）。`日志 ERROR 行数: 0`。
- **像素**：跑动排近景 6.6 m，默认幅度下开/关差异 **16.19%** 像素。
- **构建**：`_cleanrun.py … --target volunteer_army_pc` → **0 error / 0 warning**。

**没做的**：上半身持枪姿态分离（单块网格，手臂与躯干切不开）；足迹 / 扬尘；
跑动脚步声（音频层已有 `reloadStart` 这类近场音的接法，加 `footstep` 是纯增量）。

### 双腿交替（`UnitLeg`）

**为什么是顶点位移，不是蒙皮。** 三条路都评估过：

| 方案 | 代价 | 结论 |
|---|---|---|
| ① **着色器顶点位移** | 零 CPU / 零内存、约 100 行；代价是材质要换成 `ShaderMaterial` | **选它** |
| ② 运行时造骨架 + `Skin` + 权重 | 材质不用动；但要约 150 行 C++、自己算 bind 矩阵、11 个角色各多一份改过的 mesh（约 27 MB） | 出错面大得多，不选 |
| ③ 重新生成带骨骼的素材 | 最"正规"；但要重花 11 × 40 积分，且图生3D **不保证**输出骨骼 | 等于把已跑通的角色管线推翻重来，不选 |

**⚠️ 坐标口径（错一次，所有符号全反）。** 顶点数据是 Blender 的 Z-up，靠 **root 节点上的
+90°X 旋转** 才变成引擎的 Y-up（场景名 `convert`，Blender glTF I/O v4.0.43）。
所以 **着色器里 `VERTEX` 看到的是**：
...
⇒ "在髋以下"的判据是 `VERTEX.z > hip_z`（**大于**），与直觉相反。这是量出来的。
量错一次的代价：第一版体检脚本没套节点旋转，把"前后轴"当高度、"身高轴"当左右，
报出"跨中线 0%、Z 全 ≤0" —— 看着像模型性质，其实是量错了轴。

**几何权重。** 髋以下按高度线性加权：`w = smoothstep(0, 1, (z − hip_z) / band)`，
`hip_z` / `band` 都从 **mesh 的 AABB** 按比例算（默认髋高 = 身高 52% 处、过渡带 = 身高 15%）。
权重乘在**角度**上（不是矩阵插值）⇒ 髋高处 `w = 0` 连续归零，不会撕裂。
左右腿相位差 π：`ph = phase + (x < 0 ? π : 0)`。小腿可选再补一次绕膝的弯曲（默认关）。

**材质怎么办（唯一被改动的既有行为）。** 顶点位移必须挂在自定义着色器上，而角色原本用的是
Godot 从 glTF 导入生成的 `StandardMaterial3D`。所以本层会把它换成 `ShaderMaterial`，
并把三张贴图按槽位搬过去：`albedo ← TEXTURE_ALBEDO (: source_color)`、
`orm ← TEXTURE_ROUGHNESS / METALLIC / ORM`（glTF 的 MR 与 Godot 的 ORM 同布局：
G = roughness、B = metallic，所以直接按通道取）、`normal ← TEXTURE_NORMAL (: hint_normal)`。
着色器只输出 `ALBEDO / ROUGHNESS / METALLIC / NORMAL_MAP`，**光照仍由引擎管线负责**，不自己写。

> **实测：换材质在外观上是零代价** —— 开 / 关完整消融，差异 **0 像素**。所以腿摆是纯增量，
> 不存在"为了动腿而改坏了外观"。
> 而未设置的 `sampler2D` 默认值是**纯白**（G = 1 → roughness 1、B = 1 → metallic 1），
> 会渲染成一整块黑金属，所以代码里备了一张 1×1 的兜底 ORM（G = 0.8 / B = 0）。

> **一条踩过的坑：所有角色渲染路径都要挂同一套材质。** 检阅台有 `row`（陈列排）/
> `one:`（近景单体）/ `run:`（跑动排）三条路径，实跑还有第四条。**哪条漏挂，那条就退回
> 原始 `StandardMaterial3D`** —— 于是两个模式下的取证图无法互相比对，而这类偏差
> 在截图上根本看不出来。所以陈列排这种"站着不动"的路径也照样调 `leg_.apply(nd, key, 0, 0)`
> （摆角 0 ⇒ 形变恒等，但材质仍统一）。**不是为了让它动，是为了让对照成立。**

**现场可调（不重编译）**：

| 旋钮 | 默认 | 作用 |
|---|---|---|
| `VA_LEG=0` | — | 整体关闭：不载着色器、不换材质 ⇒ 画面与"加这个功能之前"**逐像素相同**（消融对照组 + 失败回退开关） |
| `VA_LEG_SWING=<度>` | 26 | 大腿摆幅。**只有 `unit_anim.cpp:71` 读这一处**，`UnitLeg` 不重复读（摆角经 `PoseVals::leg_deg` 传过去），两层因此必然同相 |
| `VA_LEG_HIP=<0..1>` | 0.52 | 髋高占包围盒高度的比例 |
| `VA_LEG_BAND=<0..1>` | 0.15 | 髋下过渡带占高度的比例（越大越"软"） |
| `VA_LEG_KNEE=<度>` | 0 | 小腿额外弯曲（关） |
| `VA_LEG_KNEE_POS=<0..1>` | 0.28 | 膝高比例 |
| `VA_LEG_PROBE=1` | — | 染色探针：左腿绿 / 右腿红、亮度 = 权重，髋以上中性灰（验髋高与左右判定） |
| `VA_LEG_PROBE=2` | — | 把**每实例参数**画成颜色（R = 相位 G = 摆角 B = 髋高） |
| `VA_DBG_LEG=1` | — | 逐键打印 H / 髋高 / 过渡带（标定髋高时看这个） |

腿摆参数用 **`GeometryInstance3D::set_instance_shader_parameter()`** 逐实例传
（比逐个克隆材质省状态提交）；`apply()` 设计为**幂等 + 每帧调用**：
节点会被整体重建（增援 / 重开一局），缓存指针必然悬空，所以每次现场在子树里找
`MeshInstance3D`（深度 ≤ 3，代价可忽略）。

**取证与验证**

| 判据 | 做法 | 结果 |
|---|---|---|
| 几何上分得开 | `_inspect_glb_legs.py` 统计腿区间跨中线三角形 | 身高 35% / 45% 两档均 **0 个（0.0%）** |
| 左右没判反 | `VA_LEG_PROBE=1` 染色截图 | 彩色区**只覆盖两条腿**，髋以上中性灰，左绿右红分明 |
| 逐实例参数送达 | `VA_LEG_PROBE=2`，跑动排 6 格 | 6 格颜色**逐格不同**（蓝→白渐变）；若同色即 `instance uniform` 没逐实例生效 |
| 材质是零代价 | 开 / 关完整消融（6 格）做差分 | **0 像素**（差分图逐像素全黑，`YAVG=0`；`sweep/` 不入库，用 `tools/capture_leg.sh` 重跑） |
| 腿真的在摆 | `VA_UNIT_SHOW=run:<键>` 按相位均分摆一排，做差分 | 相位 0° / 180° 两腿并拢，60° / 120° / 240° / 300° 前后分开且左右交替；差异 7329 px，**全落在下半身**（`sweep/leg_rundiff.png`） |
| 轴口径自洽 | `[leg]` 从 mesh AABB 量的 H 对 `[unit]` 在引擎空间量的 H | `1.091` vs `1.0906774` —— 吻合 |
| 战场实跑 | `VA_FF=6 VA_SCRIPT_A=1 VA_AUTO=1` | 11 个角色键**全部挂载**、三张贴图全 `true`、`ERROR 0` |

取证脚本 `tools/capture_leg.sh`，三种模式 `run` / `probe` / `one`，
默认定住 `VA_SEED=1 VA_HIDE_HUD=1 VA_VM_HIDE=1` —— 后两条是必需的：
HUD 罗盘 / 雷达 / 闪烁与枪模摇摆都是**墙钟动画**，不摘掉会把差分全污染成噪声。

## 射击光效（枪口焰 / 曳光弹 / 弹着火花）

**需求**：敌方射击时要有光效，能分清是哪些敌人在射击。

**做法**：新加一个纯表现层 `src/node/fx_layer.{h,cpp}`（`FxLayer`），
**`src/sim/` 一行未改** —— 逻辑层从移植第一天起就在发这些事件，此前表现层一条都没消费
（接入前 `grep 'W.fx\|W.projectiles' src/node/` 是**零命中**）。

### 三件事分别从哪取数

| 光效 | 数据来源 | 判据 |
|---|---|---|
| 枪口焰 | **`W.projectiles`**（不是 `W.fx`） | `p.t <= window(kind)` |
| 曳光弹 | `W.projectiles` | `is_tracer_kind` ⇒ `Bullet` / `TankMG` |
| 弹着火花 | `W.fx` 里 `type=="spark"` | 区间与 `life` 相交 |

⚠️ 枪口焰为什么绕开 `W.fx` 的 `flash` 条目：**`FxItem` 没有 `team` 字段**，
它回答不了"这是谁打的"，而本需求的核心恰恰是这个。改用"子弹刚出膛"当判据：
子弹出生点 = 枪口前方 8 逻辑单位，与 flash 的 x/y **逐位相同**；窗口按弹种对齐
flash 的 `life`（步枪 0.055 / 车载机枪 0.050 / 手雷 0.080 / 火箭 0.120 / 车载炮 0.160 s）。
这样既忠于逻辑层时长，又**自带阵营**。

⚠️ 弹着火花只能中性色：spark 是子弹**命中那一帧**产生的，而那颗子弹在**同一次
`update_projectiles` 调用里**就被 erase 了 ⇒ 下一帧表现层拿不到它的 `team`。
火花回答的是"打到哪"不是"谁在打"，中性暖白足够。

### 玩家自己的弹道起点 = 第一人称枪模的枪口

**需求**：「我发射的子弹的弹道应该是从我持有的枪的枪口处发出。」

**症状**：玩家自己那发弹原本也走"逻辑坐标 → 地面 + `kTracerH`"这条通用口径，
于是弹道线从**画面下方**凭空冒出，跟右下角枪模的枪口对不上。量出来的差：
第一人称相机架在玩家单位**正上方 1.65 m**，而 `muzzle` 经 `hip_pos` + 腰射姿态换算后
落在相机**前方约 0.8 m、右侧 0.1 m、高约 1.6 m**；逻辑层给的子弹出生点却是
"单位前方 8 逻辑单位（0.4 m）"、映射后**离地 1.25 m**。
前端相对相机是"前 0.4 m、下 0.40 m" ⇒ 屏幕下方约 **45°**，已经**在画面之外**
（垂直半 FOV 只有 32.5°）。只有子弹飞远后的那一小截才进画面，看起来就是一条**悬空的断线**。

**为什么不能在逻辑层改**：`src/sim/` 是**纯 2D 平面**（只有 x / y，没有高度），
"枪口"在它那里只能是"单位前方 N 单位"；而枪口在 3D 里的高度与横向偏移取决于
第一人称姿态（腰射 / 开镜是两套 viewmodel 变换），那是表现层的东西。
所以这里只**读出**一个坐标给表现层，`src/sim/` 一行不动。

**做法**（三层，全是只读）：

| 位置 | 内容 |
|---|---|
| `ViewModel::muzzle_world(Vector3&)` | 读 `muzzle->get_global_position()`；节点不在树上时返回 false |
| `FxLayer::set_player_muzzle(bool, Vector3)` | `WorldSim` 每帧在 `step()` 之前喂进来 |
| `FxLayer::build_tracers` 的玩家分支 | 以枪口为原点，沿**逻辑弹道方向**按"已经飞出多远"截取 `kTracerLen` 一段 |

⚠️ **玩家分支不能照搬通用路径的"从子弹位置往回退 2.3 m"**：玩家那颗弹出膛后前几帧
仍在相机前方 0.4 m 处，往回退会得到一段**穿过玩家自己身体**的线（起点跑到相机背后去）。
按"飞了多远"截取天然满足"起点**永不早于**枪口"，且第一帧就是从枪口射出的一小段。
`p.t` 在 spawn 时被清零 ⇒ 出膛那一帧用 `kOwnHeadMin = 0.60 m` 兜底，
否则条带退化成零长度、那一帧什么都不画（而那恰恰是最该被看见的一帧）。

⚠️ **枪口坐标必须校验"它真的还长在这个玩家手上"**：枪模挂在**相机**下，而检阅台
（`VA_UNIT_SHOW`）与菜单外壳会把相机摆到地图别处，枪口于是跟着相机一起跑 ——
不校验的话，玩家那发弹的线会从几十米外长出来（比原症状更糟）。
判据用**逻辑单位**的水平距离 `< 60`（= 3 m；正常第一人称下这个值是 0.8 m 上下）。

**旋钮与诊断**

| 旋钮 | 默认 | 作用 |
|---|---|---|
| `VA_FX_PMUZ=0` | — | 关掉这条路径，玩家弹道回退旧口径 —— 用来做"确实修的是这一条"的消融对照 |
| `VA_FX_TNW=<弧度>` | 0.012 | 曳光弹**近处宽度上限**（≈0.7°，约 11 px）—— 没有它近处是一块 43 px 的白片 |

`VA_DBG_FX=1` 的日志里多了三件事。**「弹道还是从画面下方冒出」有两种根因**，
只报一个数分不出来（枪口压根没喂进来 vs 喂进来了但玩家这一帧没开枪）：

```
[fx] … · 玩家枪口 px 1187,732 · 距相机 0.79m · 本帧走枪口路径 1
[fx] … · 玩家枪口 **未喂进来**（枪模未建好 / VA_VM_HIDE=1）⇒ 玩家弹道走旧口径
```
...
| 判据 | 做法 | 结果 |
|---|---|---|
| 构建 | `_cleanrun.py … --build build --config Release --parallel` | **0 error / 0 warning**；dll 1,445,888 B（+2,560 B） |
| 引擎加载 | `--headless --path . --quit` | `EXIT=0`、**ERROR 0**，就绪行含 `玩家弹道起点=枪口true` |
| 枪口坐标确实喂进来了 | `VA_DBG_FX=1` 的日志 | `玩家枪口 px 1145,628 · 距相机 0.81m` |
| 玩家自己的弹确实走了新路径 | 同上 | `本帧走枪口路径` 峰值 **6**（17 行 > 0） |
| **弹道确实改到了枪口** | `VA_FX_PMUZ` 0/1 同刻像素差分（`diff` 模式 + `ff=1`） | 差异 **987 px（0.21%）**，且**只有两处** —— 旧的悬空竖线 + 新的枪口段；其余**逐像素全黑** |
| 观感 | `look` 模式第一人称实拍 | 白光从枪口沿枪管射出（`sweep/v_fx_look_3795655348/cap_200s.png`） |

> ⚠️ 弹道方向与视线**接近平行**时（第一人称朝正前方射击就是这种情形），2.3 m 的条带
> 在屏幕上投影成**一小块**而不是一条长线 —— 这是透视的必然，不是"没画出来"。
> 所以判据要看 `VA_FX_PMUZ` 的 **0/1 差分位置**（旧的悬空段消失、新的枪口段出现），
> 不要只看单帧"线够不够长"。
>
> 顺带修掉的一个观感缺陷：条带宽度原先只有"世界尺寸下限 + 远处角宽下限"，
> 近处（1 m）恒为 0.045 m ≈ **43 px**，读起来是一块"贴在枪口的白片"。
> 加了近处**角宽上限**（`VA_FX_TNW`，默认 0.012 ≈ 11 px）后，玩家自己那段才读成弹道。

### 阵营配色

| 阵营 | 颜色 | 出处 |
|---|---|---|
| 友军 | 冷青蓝 `(0.30, 0.78, 1.00)` | 网页版曳光弹的色相（`index.html:4583`），**饱和度加重** |
| 敌军 | 暖橙红 `(1.00, 0.38, 0.10)` | 同上 |
| 弹着火花 | 中性暖白 | 拿不到阵营，见上 |

探针 `VA_FX_PROBE=1` 会把两色换成友军绿 / 敌军红 / 火花蓝，用来验证"颜色确实按 team 走"。

### 排查出来的四个真缺陷（都不是"调参"，是 bug）

这四条合起来才是"看得到 **且** 分得清"；少任何一条，画面表现都只是"没看见光"：

1. **曳光弹长度单位混用** —— `kTracerLen = 2.30`（**米**）被直接减在 `p.x`（**逻辑坐标**）上，
   而 1 逻辑单位 = `S = 0.05 m` ⇒ 实际长度只有 **0.115 m**，短了一个数量级。
   日志与计数一切正常（`[fx] 曳光弹 24`），**只有看画面才发现**。
   与音频层"SND 的 ref/max 是逻辑单位、必须 ×S"是同一类坑。
2. **光效被雾吞掉** —— 场景开了体积雾（`scene_builder.cpp` 的 `set_volumetric_fog_enabled`），
   雾是后处理、对 unshaded 材质照样生效 ⇒ 近处清清楚楚、远处什么都不剩。
   加 `FLAG_DISABLE_FOG`（关掉深度雾那一半；体积雾关不掉，靠下一条撑）。
3. **世界空间固定尺寸** ⇒ 远处只有 1~2 px（曳光弹宽 0.045 m 在 50 m 外是亚像素）。
   按距离给**最小角尺寸**：`size = max(size, dist × ANG)`，屏幕上占的像素数才不随距离塌。
4. **阵营色被 ACES 压成白** —— 光效走"加色混合 → HDR → ACES"，大增益下三通道一起被压向 1.0，
   冷青与暖橙**映射出来都是白**。两条对策：色相推得更极端；曳光弹**改用普通 alpha
   混合（MIX）**而不是加色 —— 加色在亮背景上必然偏白（`dst + src`，背景越亮越快饱和），
   增益调到多低都没用。代价是少一点"发光"观感，可读性优先。

### ⚠️ 这条链路上最容易漏的两类失败

它们**都表现为"画面上没有光"**，但根因完全不同，只看截图分不出来：

- **「一个光效都没画」vs「画了但全被近距剔除」** ⇒ 看 `dump()` 里的 `近距剔除 N`。
- **「在相机背后」vs「在正面但太小 / 被雾吞」** ⇒ 看 `dump()` 里的 `正面 N/背后 M`。
  （实测近景机位 **正面 370 / 背后 1** ⇒ 朝向没问题，往尺寸与雾上找。）

### 旋钮（都不必重编译）

| 旋钮 | 默认 | 作用 |
|---|---|---|
| `VA_FX=0` | — | 整体关闭：不建节点、不遍历（消融对照组 + 失败回退开关） |
| `VA_FX_MUZ=0` / `VA_FX_TRACER=0` / `VA_FX_SPARK=0` | — | 分层消融，确认某一类光效的独立贡献 |
| `VA_FX_GAIN=<n>` | 1.35 | 枪口焰 / 火花的 HDR 增益，刚过场景 glow 门槛（1.05，`scene_builder.cpp:2292`） |
| `VA_FX_TGAIN=<n>` | 1.15 | **曳光弹单独一个增益**，低于枪口焰 —— 亮度让位于可辨性 |
| `VA_FX_ANG=<弧度>` | 0.020 | 枪口焰 / 火花**最小角尺寸**（≈1.15°），见上面第 3 条 |
| `VA_FX_TW` / `VA_FX_TL` | 0.0030 / 0.016 | 曳光弹**最小角宽 / 长**（远处看得见） |
| `VA_FX_TNW` | 0.012 | 曳光弹**近处最大角宽**（≈11 px）—— 见上一节，玩家自己那段的唯一受益者 |
| `VA_FX_NEAR=<米>` | 1.5 | 近距剔除半径（**玩家自己的枪口焰**不画 —— 枪模上另有独立闪光；玩家曳光弹仍然画，见上一节） |
| `VA_FX_PMUZ=0` | — | 关掉"玩家弹道从枪口出发"，回退旧口径 —— 上一节那条改动的消融对照 |
| `VA_FX_PROBE=1` | — | 阵营染色探针：友军绿 / 敌军红 / 火花蓝 |
| `VA_DBG_FX=1` | — | 每 0.5 s 一行：各类数量 + 峰值 + 敌我分布 + **正面 / 背后累计** |

### 取证与验证

脚本 `tools/capture_fx.sh`，三种模式 `look` / `diff` / `probe`；
差分定位用 `tools/fx_pair.py`（成对逐帧相减，并报**差值符号** —— 光效是发光体，
开→关应当"变暗"，符号反了就说明差在别处而不是光效），放大看细节用 `tools/fx_zoom.py`。
目录名带**采样窗口指纹**（含 `VA_FF`、机位、关键旋钮）—— 不同批次天然隔离。
之所以不用"先清空再拍"：这台机器上 bash 的 `rm` 被安全策略拦掉（FAIL_CLOSED，
静默不执行），清空这一步会悄无声息地失效、两批截图混在一处
（2026-09-25 踩到：对照组 34 张 vs 实验组 24 张）。

| 判据 | 做法 | 结果 |
|---|---|---|
| 构建 | `_cleanrun.py … --build build --config Release --parallel` | **0 error / 0 warning**；dll 1,443,328 B |
| 引擎加载 | `--headless --path . --quit` | `EXIT=0`、**ERROR 0**、`[fx] 射击光效就绪` |
| 光效确实在生成 | `VA_DBG_FX=1` 的峰值 | 枪口焰 **15**、曳光弹 **25** |
| 朝向正确 | `dump()` 的正面 / 背后累计 | **正面 370 / 背后 1** |
| 阵营分色机制生效 | `VA_FX_PROBE=1` | 友军**绿**斑清晰（178s 2138 px） |
| 曳光弹可见且是实色 | `look` 模式近景（伏击起爆时刻） | **青蓝色**枪口焰光斑 + 曳光线（不再是白线） |
| 消融可分辨 | `VA_FX=0` 同名帧差分（`ff=1`） | 8 帧有光效（12~5433 px）、16 帧 0 px，bbox 集中在公路 / 弹道区 |

> ⚠️ 差分**必须**用 `VA_FF=1`：`ff>1` 时逻辑推进依赖墙钟帧率，两组会对不齐。
> 实测 `ff=3` 时 176 / 178 两帧差到 17%~34% 像素 —— 那是**帧没对齐**，不是光效；
> `ff=1` 下同两帧降到 1.13% / 0.21%。

## 任务介绍语音（简报念出来）

主菜单到进关之间，屏幕上一直有字，耳朵里一声没有。这一层把简报上那几行
「这是哪、打谁、干什么」念出来 —— 进简报页自动播一遍，**B** 键随时重播。

### 素材是烘的，不是录的

与音效同一条路线（`tools/gen_sfx.py`）：`tools/gen_voice.py` 用**本机 Windows 的
离线 TTS** 合成 —— OneCore 的男声 **Kangkang**（`Windows.Media.SpeechSynthesis`），
拿不到时回退 SAPI5 的 Huihui。后处理与其它素材同一口径：44100 Hz / 16-bit / mono、
峰值拉到 0.95。

**为什么不用在线 TTS**：本机到 `speech.platform.bing.com:443` 是不通的
（实测 `ClientConnectorError`，与 GitHub 同一个毛病）。离线那条路另有好处：
素材可重跑、可追溯，换台机器也能一模一样地再烘一遍。

| 产物 | 说明 |
|---|---|
| `assets/audio/vo_*.wav` | 6 条：菜单开场白 + 五关各一条 |
| `src/node/voice_lines.h` | **脚本一并生成**的 id / 文本 / 时长表 |

第二条是这次特意加的：口播稿写在脚本里，但 C++ 侧也需要知道"有哪几条、文本是什么、
多长"（日志要打名字、校验要报时长）。让它跟 wav 一起生成，
**"念出来的"与"写下来的"就永远是同一句话**，不会各写一份慢慢漂移。

脚本还会校验稿子里的 id 在 `va_campaign.cpp` 的关卡表里真实存在 ——
关卡改名之后忘记重跑，这一步会直接报错，而不是等实机上发现"怎么没声音"。

### 五条链路

| 时机 | 行为 |
|---|---|
| 进主菜单 | 念 `vo_menu`（**整场只念一次**，从结算回菜单不重复念） |
| 进简报页 | 念本关那条，每次进都念（重打 / 转进 / Esc 回来重看，都需要再听） |
| 按 Enter 开打 | **停掉**没播完的 —— 战斗第一枪不该被人声盖住 |
| 战斗中按 **B** | 重播本关介绍。部署期长达 150 秒，够听好几遍 |
| 转进下一阵地 | 换关时自动念新阵地那条（**重打本关不念**） |

### 为什么单独一层，不塞进 `Audio`

战斗音效层的需求与语音正好相反：

- **独占**：同一条不该叠着播，新的来了旧的就得停（`Audio` 是"随便挑一个空闲声部"，
  不保证独占，可能出现两句话叠在一起）；
- **不空间化、不抖 pitch**：这是旁白，不是世界里某个东西发出来的声音；
- **长**：一条 15~18 秒，塞进共享声部池会长期占着一个格子，最激烈的时候少一路枪声。

所以 `node/voice.{h,cpp}` 起了一条自己的 `AudioStreamPlayer`，走 **Master** 总线 ——
SFX 总线上挂着限幅器，齐射时它会把旁白一起压下去。

顺带把音效层里"先试 `ResourceLoader` 再 `AudioStreamWAV::load_from_file`"那段
提成了公共函数 `load_wav_resource`（声明在 `audio.h`）—— 本工程的 wav 从不走编辑器导入、
`res://` 下没有 `.import`，这个坑语音层要再踩一遍，不如共用一份实现。

### 键位：为什么是 B 不是 V

V 已经是"切武器外观"了。剩下的字母里 briefing 的首字母最能自解释，
也不与任何移动 / 射击键相撞。
...
| 旋钮 | 作用 |
|---|---|
| `VA_VO=0` | **整体关闭**：素材不载、声部不建 —— 与加这一层之前**逐像素相同** |
| `VA_DBG_VO=1` | 报载入结果 + 逐条时长（与脚本烘素材时记的对照）+ 每次播报的 id 与稿子原文 |

### 取证与验证

脚本 `tools/capture_vo.sh`，六种模式：

```bash
PY="C:/Users/lujiajing/.workbuddy/binaries/python/envs/default/Scripts/python.exe"
"$PY" tools/gen_voice.py --check     # ① 字节层：采样率 / 时长 / 峰值
bash tools/capture_vo.sh off         # 关掉整层的对照（headless）
bash tools/capture_vo.sh shot        # 简报页：截图 + 进页自动播报
bash tools/capture_vo.sh replay      # 简报页按 B
bash tools/capture_vo.sh entry       # 按 Enter 进关，旁白被停掉
bash tools/capture_vo.sh inplay      # 战斗中按 B
bash tools/capture_vo.sh next        # 转进下一阵地，新关介绍自动念
```

判据分三层，一层比一层强：**字节层**（采样率 / 时长 / 峰值）→
**引擎层**（`[vo] 语音载入 6/6`，且逐条时长与脚本记的对得上）→
**链路层**（真按下去 / 真换关，日志里出现 `[vo] 播放 <id>` 带稿子原文）。
前两层 headless 就能拿；第三层必须带窗口 —— headless 用的是 dummy 音频驱动，
**能跑但没声**。

| 判据 | 结果 |
|---|---|
| 构建 | **0 error / 0 warning**；dll 1,472,000 B |
| 素材校验 | 6/6 通过（44.1 kHz、峰值 0.944~0.953、14.6~18.4 s）|
| headless 加载 | `EXIT=0`、**ERROR 0**；`[vo] 语音载入 6/6`、`[snd] 音效载入 25/25`（重构没伤到音效层）|
| 时长逐条对得上 | 6 条全部「引擎解析时长 == 脚本记录时长」（18.43 / 17.81 / 14.64 / 16.88 / 18.39 / 6.78 s）|
| 进简报自动播 | `[ui] 初始屏 = BRIEF` → `[vo] 播放 l1_yunvfeng (18.43s)` + 稿子原文 |
| 按 B 重播 | 简报页投 B → **播放次数 2** |
| 进关停掉 | 投 Enter → `[vo] 进入战斗 —— 停掉没播完的任务介绍`，播放次数仍是 1 |
| 战斗中按 B | `VA_SCREEN=play` 投 B → `[vo] 播放 l1_yunvfeng` |
| 转进换关自动念 | 撤退 → 结算 → 按 R → 第 2 关 → `[vo] 播放 l2_233` |
| 关掉整层 | `VA_VO=0` → 只有一行「语音层关闭」，没载素材、没建声部 |

> ⚠️ **`kill` 掉进程时那几条 ERROR 不算回归**：`Unreferenced static string` /
> `RID allocations … leaked at exit` / `PagedAllocator` 都是引擎在清理阶段自己吐的，
> 本层一条都不会产生（`--headless --quit` 那条干净路径是 0 ERROR）。
> `capture_vo.sh` 已经把这三类过滤掉了。

**取证时踩到的两个坑（都写进脚本注释了）**

1. `replay` / `entry` 模式忘了锁初始屏 → 停在主菜单，播的是菜单开场白；
   按 B 也没反应（B 只在简报页认）。日志里"播放次数 = 1"看着像**重播没生效**，
   其实是压根没走到那一屏。
2. `next` 模式只投了一次键 → **转进是两步**：投 `1` 只是选撤退、弹出结算面板，
...
- **界面外壳**：主菜单 / 任务简报（区域态势图 + 任务目标 + **小队名册胸像**，格数按**当前花名册**现算，
  战役里每关 10 格）/ 结算面板，全部手绘
...
- **五个伏击阵地**：把"一局"接成"一条线" —— 玉女峰 / 233.2 高地 / 种子山 / 鹫峰 / 沙宗洞，
  是 63 军在铁原以南逐次抵抗的五个阻击位置，**一个阵地打完了，带着活下来的人去下一个阵地**。
  五关**只有一条主目标：拖住敌人 240 秒**（不夺密码箱、不炸坝），难度靠递增的敌军
  （步兵 10~14 → 16~22）与地形（山口 / 丘陵 / 孤山 / 峡谷）堆，不靠加秒数。
  每个阵地一份 **10 人花名册**（共 50 个名字，模型复用 11 套 `char_*`），
  `ROSTER` 因此从编译期常量表改成运行时容器，分组 / 职务呼号 / 别名 / HUD 胸像四处跟着现算。
  三条链路各有独立出口：**伤亡过半**弹常驻选择条（1 撤 / 2 守）、**玩家阵亡**直接转进下一阵地
  并由该阵地队长接任、**熬满 240 秒**转进。详见「五个伏击阵地」
...
- **任务介绍语音**：6 条人声（菜单开场白 + 五关各一条），进简报页自动念、
  按 **B** 重播、转进下一阵地自动念新阵地那条。素材由 `tools/gen_voice.py`
  用**本机离线 TTS**（OneCore 男声 Kangkang）烘到 `assets/audio/vo_*.wav`；
  文稿与 id 表由**同一个脚本**产出 `src/node/voice_lines.h`，保证"念出来的"
  与"写下来的"是同一句话。播放走独立的一层 `src/node/voice.{h,cpp}`
  （独占声部、不空间化、不抖音高，与战斗音效互不打扰），
  `VA_VO=0` 可整体关掉。详见「任务介绍语音」
- 单位动作：**两层都已接入**，逻辑层 `src/sim/` 一行未改。
  - **整体跑动节奏**（起伏 / 前倾 / 左右摇 / 肩摆 / 点头，随实测地速变化）走
    `src/node/unit_anim.cpp`，`VA_RUN=0` 可整体关。
  - **双腿交替**（髋以下按高度权重绕横轴摆，左右相位差 π）走
    `shaders/unit_leg.gdshader` + `src/node/unit_leg.cpp`，用着色器顶点位移实现 ——
    **不需要骨架**，`VA_LEG=0` 可整体关（回到原始材质，逐像素等同于未加本层）。
  - 见「单位动作（跑动节奏 + 双腿交替）」一节。**仍未做的**：上半身持枪姿态分离
    （单块网格，手臂与躯干切不开）、足迹 / 扬尘、跑动脚步声。
- 射击光效：**枪口焰 / 曳光弹 / 弹着火花已接入**，走 `src/node/fx_layer.{h,cpp}`，
  逻辑层 `src/sim/` 一行未改（逻辑层早就在发这些事件，此前表现层零消费）。
  枪口焰与曳光弹按**阵营分色**（友军冷青蓝 / 敌军暖橙红），`VA_FX=0` 可整体关。
