    ...
    "prop": {
        ...
        "prompts": {
            ...
            "prop_pine3":
                ...
                "朝鲜战场山地常见的东北红松，写实摄影，纯白背景，只拍这一棵树",
            # 【2026-09-24 三补：树干是品红色 —— 这条是 v1 的 prompt **自己招来的**】
            # 症状：出生点就在树底下，第一人称一抬头就是**一整块品红平板**（树干），
            # 与满屏的灰绿/土黄刺眼地打架。它从 09-23 那版起就在，一直没人发现 ——
            # 因为上一轮的判据只判**形状**（剖面锯齿度 0.2120→0.0268 全过）。
            #
            # 根因有两层，第二层是本轮的发现：
            #   ① 生成器给的 baseColorTexture **底色本身就是暗红**（导出来看过，
            #      红底 + 灰绿枝叶团块、像迷彩）—— 不是引擎问题、不是 tint 问题；
            #   ② **prompt 里明写着"红褐色树干"**，末尾还写着"东北**红**松"。
            #      生成器忠实执行了它。"红褐色"在 PBR 反照率上落地成了高饱和暗红。
            # 所以这不是"生成器抽风"，是**描述里真的带着色相指令**。
            #
            # 修法与上一轮"一摞飞盘"同一条方法论：**改描述里的色相词，不是加"不要红"**。
            #   prop_pine4 = v3 的**形状措辞逐字保留**（"密实饱满的圆锥体 / 蓬松浓密
            #   如绒团 / 轮廓连续光滑均匀收尖 / 没有横向伸出的枝盘"是上一轮把锯齿度
            #   打到 0.0268 的功臣，一个字都不动），**只做颜色手术**：
            #     · "笔直的红褐色树干" → "树皮是粗糙纵裂的木质，深灰褐色混着青灰与土黄"
            #     · 正面给饱和度："颜色灰暗朴素不鲜艳"
            #     · "东北红松" → "松树"（"红"这个字本身也是色相线索）
            # ⚠️ 形状措辞一个字不改的理由：重新生成 = **重掷骰子**，上一轮的几何成果
            #    （1/7.9 的锯齿度）是这个键上最贵的东西，不能为了换颜色把它赔进去。
            #
            # 并行的保底通道：`tools/glb_tex_recolor.py`（**零额度**，直接改贴图上
            # 红色区域的色相，几何一个顶点都不动）。两条路的产物都过
            # `tools/glb_tex_color.py` 的颜色判据，取优。
            "prop_pine4":
                "一棵高大的针叶松树单体，笔直粗壮的树干，树皮是粗糙纵裂的木质，"
                "深灰褐色混着青灰与土黄、颜色灰暗朴素不鲜艳，"
                "树冠是一个密实饱满的圆锥体，通体被极细的针叶紧密覆盖，"
                "质感蓬松浓密得如同一个绿色的绒团，枝条完全被针叶包裹住、"
                "看不出分层也看不出任何间隙，整体轮廓连续光滑、从底部到顶端均匀收尖、"
                "没有横向伸出的枝盘，朝鲜战场山地常见的松树，写实摄影，纯白背景，只拍这一棵树",
            "prop_bush":
