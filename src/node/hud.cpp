...
//  准星
// ===========================================================================
/* 常驻准星。**取证判据提醒**：这里画的是中心圆点 + 上/下/左/右四条**正交**短线。
   而"命中标记"是四条 ±45° **斜线**（见下面的 draw_hitmarker）。
   所以看截图判定"命中路径有没有生效"时，**"中央有十字"不算数 —— 要看有没有斜线**。
   本条是复核时补的：命中标记那四条斜线很细，第一眼极易当成准星自带的装饰。 */
...
//  命中标记
// ===========================================================================
/* 只在命中/击杀事件后短暂出现：命中 0.24 秒、击杀 0.40 秒，**都是墙钟秒**
   （衰减用的是 update() 传进来的 p_delta，不是逻辑层的固定步）。
   形状是四条 **±45° 斜线** —— 这是相对常驻准星（draw_crosshair 的正交十字）
   唯一的区别，也是看图取证的唯一判据；命中白色、击杀红色并带中心红点。
   取证时"等多久再截图"必须小于这个寿命，见 world_sim.h 的 ev_wait_sec_。 */
