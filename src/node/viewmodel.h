...
struct ViewModel {
    ...
    // 实测定档（`sweep/vmhand_hip2/sweepS.sh` + `verify.sh`）：
    //   缩 0.65 + 让 (0.06,-0.04,0.02) → 三把枪开镜时"手到瞄准点"分别是
    //   莫辛 124 px / 波波沙 88 px / DP-27 124 px，**中心 80 px 内一律 0 px**。
    // 为什么不取更小的缩小比例：0.50 时波波沙只剩 65 px（仍压住照门，中心 80 px 内
    // 还有 284 px 是手）—— 让位量是三把枪共用的一个常量，而三把枪的照门高度差了一倍多
    // （莫辛 0.075 / DP-27 0.160），所以这个常量只能按**最挤的那把**定，不能按莫辛定。
    // 也不取"只缩不挪"：那只剩 36% 的手像素，开镜时手像被切碎。
    float          hand_ads_shrink = 0.65f;   // 开镜时双手缩小的比例
