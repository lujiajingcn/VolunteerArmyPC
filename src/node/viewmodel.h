...
struct ViewModel {
    ...
    // ---------------------------------------------------------------------
    // 【腰射的"枪口指向"：由"枪管必须穿过准星"解出来，不再手调角度】
    //
    // 玩家报的症状：射击时"枪口指向"和鼠标位置对不上。
    // 根因在 hip_rot 那一组 (3°,13°,-6°)：它是照参考图**构图**摆的 —— 13° 的
    // 偏航让枪身斜着扫过右下象限，读起来像"端着枪"。但**枪管的方向也跟着偏了
    // 13°**：屏幕上枪管指向准星**左侧约 180 px**（截图 1920 宽，正好 = 13° 视场角）。
    // 玩家看到的不是"构图好看"，而是"枪口指着别处"。
    //
    // 为什么不能把 13° 直接改小就完事：三把枪的枪口 z 不同
    // （莫辛 -1.082 / 波波沙 -0.693 / DP-27 -1.125），而且"让枪管穿过准星"要求的
    // 内收角**随目标距离变**（3 m 处要 2.5°，25 m 处只要 0.3°）。所以这里按约束解：
    // 让"从枪口沿枪管轴射出的直线"穿过准星（= 屏幕中心那条视轴）在 conv_d 米处的落点。
    //
    // 只解**姿态角**，hip_pos 一行不动 —— 构图（枪在画面右下、枪托出画）归位置管，
    // 指向归姿态管。两者分开之后就不会再出现"改指向顺手把构图也改了"。
    //
    // 【实测（莫辛，VA_DBG_VMAIM=1）】
    //   手调 (3,13,-6)： 枪管指向落在准星左侧 186 px → 角误差 **-12.94°**
    //   解算 (0.20,0.24,-6)： 差 3 px → **-0.19°**（波波沙 / DP-27 同值）
    //   开镜（不动）：角误差 0.00°
    //
    // 【代价：按投影**推导**的，没单独量】枪管与视轴夹角从 13.3° 收到 0.2°，
    // 透视缩短使枪管那一段的屏幕长度约 164 → 127 px（-23%），整枪 840 → ~700 px。
    // 这是"指向正确"必须付的钱：**偏得越多才显得越长**，长出来的那部分正是偏出去的那部分。
    // 想既长又准只能把枪往画面下缘外侧挪，而那会把枪托推进画面里。
    // 判据是截图：枪管在画面上仍是一段可读的斜线，不是一根短棒（见 sweep/aim_conv/）。
    float conv_d = 25.0f;        // 收敛距离（米）；0 = 关掉收敛，回到固定姿态角 hip_rot
    float hip_roll = -6.0f;      // 侧倾：绕**枪管轴**转 → 不影响指向，只定观感
    bool  hip_env_over = false;  // VA_VM_ROT 显式给过 → 整套照它算（取证/找位用）
    bool  hip_solved = false;    // 诊断：当前姿态是解出来的（true）还是手调的
    // 解"枪管穿过准星"的腰射姿态角。build 时与换枪时（枪口 z 变了）各调一次。
    void solve_hip_pose();
    // VA_DBG_VMAIM=1：每秒一行"枪口 / 枪管指向落在屏幕哪儿"，与准星并排对比。
    void dbg_aim_line();
    float aim_dbg_acc = 0.0f;
    ...
    // ---------------------------------------------------------------------
    // 【枪口的世界坐标 —— 也就是"子弹该从哪儿出发"】
    //
    // 给射击光效层用（fx_layer.h 的 set_player_muzzle）。玩家自己那发弹的**弹道起点**
    // 必须落在这里，而不是逻辑层那个"单位前方 8 逻辑单位、离地 1.25 m"的点。
    //
    // 差多少：相机（= 玩家单位正上方）抬高 1.65 m，muzzle 经 hip_pos + 姿态角换算后
    // 落在相机前方约 0.8 m、右侧 0.1 m、高约 1.6 m；而逻辑出生点映射出来是
    // "前方 0.4 m、高 1.25 m"。第一人称下这两个方向差了 30° 以上 ⇒ 玩家开枪时
    // 弹道线看着从画面下方**凭空冒出**，与右下角的枪口对不上（玩家报的症状）。
    //
    // 为什么不在逻辑层改：逻辑层是**纯 2D 平面**（x, y 两个坐标，没有高度），
    // "枪口"在它那里只能是"单位前方 N 单位"；而枪口在 3D 里的高度/横向偏移
    // 本身就依赖第一人称姿态（腰射/开镜两套 viewmodel 变换），那是表现层的东西。
    // 所以这里只**读出**一个坐标给表现层用，src/sim/ 一行不动。
    //
    // 返回 false = 枪模尚未建好或用的是程序化路径（此时调用方回退旧口径）。
    bool muzzle_world(godot::Vector3 &r_out) const;
